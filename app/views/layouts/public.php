...
<body class="min-h-screen flex flex-col justify-between bg-slate-50 text-slate-800 antialiased selection:bg-blue-600 selection:text-white">
    ...
    <!-- Top Corporate Header (Clean Minimalist Header) -->
    <header class="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200/80 shadow-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between gap-3">
            ...
            <!-- Action Buttons (Hanya Butang Log Masuk / Papan Pemuka di Kanan) -->
            <div class="flex items-center gap-2 sm:gap-3 shrink-0">
                <?php if ($isLoggedIn): ?>
                <a href="<?= url('/dashboard') ?>" 
                   class="inline-flex items-center gap-1.5 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-xl shadow-md shadow-blue-600/20 transition-all shrink-0">
                ...
                <?php else: ?>
                <a href="<?= url('/login') ?>" 
                   class="inline-flex items-center gap-2 px-4 py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold rounded-xl shadow-sm transition-all shrink-0">
